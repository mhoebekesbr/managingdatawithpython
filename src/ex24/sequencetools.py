...
class Sequence :
    ...
    def __init__(self,id=None,sequence=None,seqRecord=None,begin=None,end=None):
        self.id=id
        ...
        if seqRecord is not None:
            ...
            self.sequence=str(seqRecord.seq)
...
class CodingSequence(Sequence) :
    ...
    def __init__(self,id=None,sequence=None,seqRecord=None,translation=None):
        Sequence.__init__(self,id,sequence,seqRecord)
...
def readFastaSequencesFromFile(filename, strainCollection=None, sequenceType=NUCLEOTIDES_TYPE) :
    ...
    with open(filename) as infile :
        for entry in Bio.SeqIO.parse(infile,'fasta'):
            currentSeqId = entry.id
            if currentSeqId != '':
                ...
                if sequence is None:
                    sequence=CodingSequence(seqRecord=entry)
                    strain.addSequence(sequence)
                if sequenceType == RESIDUES_TYPE :
                   sequence.setTranslatedSequence(str(entry.seq[:-1]))
    return strainCollection


def readGenBankSequencesFromFile(filename):
    strain=Strain()
    with open(filename) as infile :
        for entry in Bio.SeqIO.parse(infile,'gb'):
            if entry.features is not None :
                for feature in entry.features:
                    qualifiers=feature.qualifiers
                    if feature.type == 'CDS' :
                        location=feature.location
                        sequenceletters=location.extract(entry.seq)
                        locustag='unknown'
                        if 'locus_tag' in qualifiers:
                            locustag=qualifiers['locus_tag'][0]
                        sequence=CodingSequence(id=locustag,sequence=sequenceletters)
                        strain.addSequence(sequence)

                        if 'translation' in qualifiers:
                            translation=qualifiers['translation'][0]
                            sequence.setTranslatedSequence(translation)
                    if feature.type == 'source' :
                        if 'organism' in qualifiers:
                            organism=qualifiers['organism'][0]
                            strainName=re.sub(r"^.* (?P<strain>\S+)$","\g<strain>",organism)
                            strain.setName(strainName)
    return strain


def __buildSequenceDictFromList(sequenceList):
    sequenceDict={}
    for sequence in sequenceList:
        sequenceDict[sequence.getTranslatedSequence()]=sequence
    return sequenceDict

def buildIdenticalSequenceMatches(seqListOne,seqListTwo):
    matchlist=[]
    dictOne=__buildSequenceDictFromList(seqListOne)
    dictTwo=__buildSequenceDictFromList(seqListTwo)

    listOneSequences=list(dictOne.keys())
    for sequence in listOneSequences:
        if sequence in dictTwo.keys():
            matchlist.append((dictOne[sequence],dictTwo[sequence]))
            del(dictOne[sequence])
            del(dictTwo[sequence])
    for sequence in dictOne.values():
        matchlist.append((sequence,None))
    for sequence in dictTwo.values():
        matchlist.append((None,sequence))
